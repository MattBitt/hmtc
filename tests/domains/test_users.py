...
def test_user_create_and_load(seeded_db, user_dict):
    ...
    assert created_user.hashed_password == user_dict["hashed_password"]
    assert created_user.jellyfin_id == user_dict["jellyfin_id"]
    assert created_user.id > 0

    loaded_user = User.load(created_user.id)
    assert loaded_user.username == user_dict["username"]
    assert loaded_user.email == user_dict["email"]
    assert loaded_user.hashed_password == user_dict["hashed_password"]
    assert loaded_user.jellyfin_id == user_dict["jellyfin_id"]

    User.delete_id(created_user.id)


def test_user_delete(seeded_db, user_dict):
