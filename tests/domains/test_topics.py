...
def test_topic_create_and_load(seeded_db, topic_dict):
    created_topic = Topic.create(topic_dict)
    assert created_topic.text == topic_dict["text"]
    assert created_topic.id > 0

    loaded_topic = Topic.load(created_topic.id)
    assert loaded_topic.text == topic_dict["text"]
    Topic.delete_id(created_topic.id)


def test_topic_delete(seeded_db, topic_dict):
    new_topic = Topic.create(topic_dict)
    Topic.delete_id(new_topic.id)


def test_serialize(seeded_db, topic_dict):
    _topic = Topic.create(topic_dict)
...
def test_update_topics(seeded_db, topic_dict):
