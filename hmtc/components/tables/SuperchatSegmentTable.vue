<template>
  <div>
    <v-dialog v-model="dialog" max-width="800px">
      <v-card>
        <v-toolbar dark color="primary">
          ...
          <v-toolbar-title
            >Edit SuperchatSegment {{ currentItem.id }}</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
          ...
          <v-container>
            <v-row>
              <v-col cols="12" sm="6" md="4">
                <v-text-field
                  v-model="currentItem.start_time_ms"
                  label="Start Time"
                ></v-text-field>
              </v-col>
              <v-col cols="12" sm="6" md="4">
                <v-text-field
                  v-model="currentItem.end_time_ms"
                  label="End Time"
                ></v-text-field>
              </v-col>
            </v-row>
    ...
    >
      ...
      <template v-slot:item.start_time_ms="{ item }">
        <span>{{ item.start_time_ms }}</span>
      </template>
      <template v-slot:item.end_time_ms="{ item }">
        <span>{{ item.end_time_ms }}</span>
      </template>
